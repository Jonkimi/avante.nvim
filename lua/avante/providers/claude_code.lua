...
M.api_key_name = "ANTHROPIC_FOX_API_KEY"
...
function M:parse_curl_args(prompt_opts)
  ...
  --   ["anthropic-version"] = "2023-06-01",
  --   -- 假设 authValue 是一个已定义的变量
  --   -- ["x-api-key"] = authValue,
  --   ["x-app"] = "cli",
  --   ["user-agent"] = "claude-cli/2.0.69 (external, sdk-py, agent-sdk/0.1.16)",
  --   -- 假设你想要将 "Bearer sky-" 作为字符串处理，或者与一个变量连接
  --   -- 如果 `sky-` 是一个变量，你需要将其定义
  --   -- ["authorization"] = "Bearer sky-", -- 如果 `sky-` 是一个变量，这里可能是 "Bearer " .. sky_variable
  --   ["Content-Type"] = "application/json",
  ...
  if request_body.temperature < 1 then
    request_body.thinking = nil
  end
  print("session_id", prompt_opts.session_id )
  return {
    ...
    body = vim.tbl_deep_extend("force", {
      ...
      max_tokens = 64000,
      temperature = 1,
      metadata = {user_id = string.format("user_73e3aaeca8736f3091d6ed4690b1cab3481a07a68acb65665fffdfe39571d5f2_account__session_%s", prompt_opts.session_id)},
      messages = messages,
      tools = tools,
      stream = true,
    }, request_body),
